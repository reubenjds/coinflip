...
<div
	class="flex flex-col text-lg justify-center place-items-center min-h-screen gap-5"
>
	{#if flip === -1}
		<select
			bind:value={selectedFlip}
	...
	<button
		disabled={selectedFlip !== flip && flip !== -1}
		class="btn btn-success"
		on:click={flipCoin}>Flip Coin</button
	>
	{#if streak !== 0}
		<h1>
			Your streak is <span class="text-2xl font-bold text-yellow-400"
				>{streak}</span
			>
		</h1>
	{/if}
	...
	{#if selectedFlip !== flip && flip !== -1}
		...
		/>
		<button
			disabled={postedStreak}
			class="btn btn-warning"
			on:click={postStreak}>Add to leaderboard!</button
		>
	{/if}
</div>
